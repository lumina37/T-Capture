...
int main() {
    ...
    // Native formats
    const auto& nativeFormatBoxes = tcap::v4l2::NativeFormatBox::createBoxes(*pDeviceBox) | unwrap;
    for (const auto& nativeFormatBox : nativeFormatBoxes) {
        const uint32_t nativeFormat = nativeFormatBox.getFormat();
        const auto& resolutionBoxes = tcap::v4l2::ResolutionBox::createBoxes(*pDeviceBox, nativeFormat) | unwrap;
